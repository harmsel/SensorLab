...
ADXL345 accelemeter;  // // sluit de accelemeter aan op I2C
...
#define NUMPIXELS 15            // Pas dit getal aan aan het aantal pixels in je ledstrip
...
void setup() {
  Serial.begin(9600);        // noodzakelijk voor het weergeven  een string of waarde tekst in de Serial Monitor
  accelemeter.powerOn();     // Zet de I2C poort open voor je Accelerometer
  pixels.begin();            // noodzakelijk voor de aangesloten ledstrip
  pixels.setBrightness(50);  // Stel de helderheid (max = 255)
}
...
void loop() {
  ...
  Serial.print("x: ");
  Serial.println(x);  // Geef alleen de waarde van x weer in de Serial Monitor


  while (x > -20 && x < 20 && eenKeer) {
    ...
    } else if (stopWatch > 2000)
      Serial.println("!!!!!!!  -- Ga maar af");
  }
  eenKeer = true;
  stopWatch = 0;
}
...
/*
  ...
  while (pixNr == 6) {  // de hoek van de fles uitgedrukt in pixels
    Serial.println(stopWatch);
    stopWatch = 0;

    if (stopWatch > 2000) {  // na 2 seconden wachten
      Serial.println("Hij zit vast in de while");
    }
...
*/
delay(40);  //voor betere leesbaarheid van de serial monitor
}
